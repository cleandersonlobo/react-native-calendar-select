	...
	_i18n(data, type) {
		...
		if (~['w', 'weekday', 'text'].indexOf(type)) {
			return (
				...
				Calendar.I18N_MAP[i18n][type][data]
			);
		}
		if (type === 'date') {
			return data.format(
				customI18n[type] || Calendar.I18N_MAP[i18n][type]
			);
		}
	}
	_resetCalendar() {
		...
		this.setState({
			...
			endWeekdayText: isEndValid
				? this._i18n(end.isoWeekday(), 'weekday')
				: '',
		});
	}
	_getDateRange() {
				...
				<View style={[styles.container, mainBack]}>
					...
					<View style={styles.week}>
						{[7, 1, 2, 3, 4, 5, 6].map(item => (
							<Text
								style={[styles.weekText, subFontColor]}
								key={item}
							>
								{this._i18n(item, 'w')}
							</Text>
